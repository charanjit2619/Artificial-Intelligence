...
class Mdp:
        ...
        for i in range(100):
            ...
            for a, b, c in states:
                ...
                if (a, b) in [(4, 4), (5, 5), (3, 2), (2, 2), (2, 3)]:
                    self.new_state_value[a, b, c] = (self.state_value[a, b, c], "No Action")
                # for accessible states, value should be updated in subsequent iterations
                else:
                    self.new_state_value[a, b, c] = max([(round(self.q_value((a, b, c), act), 2), act) for act in
                                                           ["A1", "A2", "A3", "A4"]])
                # Forming a dictionary states_best_actions to keep track of best action in a particular state
                val, act = self.new_state_value[a, b, c]
            ...
            if i < 10:
                for key, value in self.states_best_actions.items():
                    print(value)
            i += 1
        print(f'\n(Values, Best Action) after 100 iterations: {self.new_state_value}')
    ...
    def policy(self, state):
        ...
        while (c, r) not in [(4, 4), (5, 5)]:
            (v, ac) = self.new_state_value[(c, r, d)]
            print(state, ac)
