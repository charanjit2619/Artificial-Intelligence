...
def get_child_boards(player, board):
    """
    Generate a list of successor boards obtained by placing a disc
...
###############################################################################
    ...
    def max_value(player, board, depth_limit):
        ...
        for c, b in get_child_boards(player, board):
            v, a = value(next_player, b, depth_limit-1)
            l.append((v, c))
        return max(l)
    ...
    def min_value(player, board, depth_limit):
        l = []
        for c, b in get_child_boards(player,board):
            v, a = value(max_player, b, depth_limit-1)
            l.append((v, c))
        return min(l)
...
#Alpha beta pruning of minimax search for efficiency
...
###############################################################################
    ...
    def max_value(player, board, depth_limit, alpha, beta):
        l = []
        for c, b in get_child_boards(player, board):
            v, a = value(next_player, b, depth_limit-1, alpha, beta)
            if v >= beta:
                return (v, c)
            l.append((v, c))
            alpha = max(alpha, v)
    ...
    def min_value(player, board, depth_limit, alpha, beta):
        l = []
        for c, b in get_child_boards(player,board):
            v, a = value(max_player, b, depth_limit-1, alpha, beta)
            if v <= alpha:
                return (v, c)
            l.append((v, c))
            beta = min(beta, v)
    ...
    score, placement = value(max_player, board, depth_limit, -math.inf, math.inf)

###############################################################################
    return placement

#Expectimax search with uniform chance of selecting available actions
def expectimax(player, board, depth_limit):
    ...
    def exp_value(player, board, depth_limit):
        ...
        for c, b in get_child_boards(player, board):
            v, a = value(max_player, b, depth_limit - 1)
            l = prob_action(player, board)*v + l
        return (l, None)
